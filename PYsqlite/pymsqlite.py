...
print('\nRECUPERANDO USUARIOS APARTIR DE CONDIÇAO DE FILTRAGEM')
for user in session.scalars(stmt):
    print(user)

stmt_address = select(Address).where(Address.user_id.in_([2]))
print('\nRECUPERANDO OS ENDEÇOS DE SANDY')
for address in session.scalars(stmt_address):
    print(address)

order_stmt = select(User).order_by(User.fullname.desc())
print('\nRECUPERANDO INFO DE MANEIRA ORDENADA E DECRESCENTE')
for result in session.scalars(order_stmt):
    print(result)

stmt_join = select(User.fullname, Address.email_address).join_from(Address, User)
print('\nRECUPERANDO INFO DE MANEIRA ORDENADA E DECRESCENTE, UTILIZANDO JOIN')
...
print('\nEXECUTANDO STATEMENT A APARTIR DO CONNECTION')
